...
def get_token_auth_header():
    ...
    # raise an AuthError if auth header is null
    if not auth:
        raise AuthError({
    ...
    # auth header consists of 2 components: Bearer + 'jwt token'
    parts = auth.split(' ')

    # the first part should be Bearer
    ...
    # auth header must include exactly 2 parts
    elif len(parts) == 1:
        raise AuthError({
            'code': 'invalid_header',
